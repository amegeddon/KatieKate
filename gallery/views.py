...
def all_gallery_images(request):
    ...
    if request.GET:
        ...
        if "sort" in request.GET:
            ...
            if sortkey == "title":
                sortkey = "lower_title"
                gallery_images = gallery_images.annotate(
...
def gallery_detail(request, image_id):
    ...
    Returns:
        HttpResponse: The rendered 'gallery/gallery_detail.html'/
...
def add_gallery_image(request):
    ...
    This view handles form submission to add a new gallery image. Only /
    superusers are allowed to add images. After a successful form submission, /
    the user is redirected to the gallery page.
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.error(
                request,
                "Failed to add Gallery Item. Please ensure the form is valid.",
            )
    else:
        form = GalleryForm()
...
def edit_gallery_image(request, image_id):
    ...
    This view allows authenticated (superuser) users to edit the details /
    of an existing gallery image, including its title, description, and /
    category. After successful update, the user is redirected to the gallery.
    """
...
def delete_gallery_image(request, image_id):
    ...
    This view allows authenticated (superuser) users to delete a /
    gallery image from the database. After the image is deleted, the /
    user is redirected to the gallery page.
    """
...
def gallery_full_view(request, image_id):
    """
    A view to display an individual gallery image in full view with /
    next/previous navigation. This view shows a specific image in full view /
    and provides navigation links to the previous and next image in the /
    gallery, based on the image ID.
    """
